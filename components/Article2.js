...
function Article2Page({ inputId }) {
  ...
  useEffect(() => {
    if (categorieRecuperee) {
      fetch(
        `http://localhost:3000/articles/articlesSimililaires?categorie=${categorieRecuperee}&type=${typeRecupere}`
      )
        .then((response) => response.json())
        .then((articlesSimililaires) => {
          if (articlesSimililaires.result) {
            // console.log(
            //   "Données d'articles similaires récupérées :",
            //   articlesSimililaires
            // );
...
if(articleCliqueData.colors9.length > 0) {
  ...
  const articles = () => {
    ...
    const sizeOrGiSize = () => {
      ...
      //   const mappedGiSizes9 = articleCliqueData.giSizes9.map(
      //     (sizeGi, index) => (
  ...
  return (
    <div className={styles.articleContainer}>
      ...
      <div>
        {articleCliqueData ? (
          <Articleliste
            _id={articleCliqueData._id}
            categorie={articleCliqueData.categorie}
            type={articleCliqueData.type}
            model={articleCliqueData.model}
            photos9={articleCliqueData.photos9}
